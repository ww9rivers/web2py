...
DIRS="comment     dialog
display     edit        fold        hint
mode     search      selection"
BINFILES=""
ETCFILES=""
LIBFILES=""
...
export EMPFBIN=${EMPFBIN:-'/opt/miops/web2py/applications/admin/static/codemirror'}/${SUBDIR:-addon}
...
for xhost in "$EMPFHOST1" "$EMPFHOST2" "$EMPFHOST3" ; do
    ...
    #chmod o-rwx $BINFILES $ETCFILES $LIBFILES
    if [ "$BINFILES" != "" ]; then
